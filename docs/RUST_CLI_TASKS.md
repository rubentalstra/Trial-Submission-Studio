...
8. Phase 7: implement domain processing and synthesis
...
- [ ] Verify standards sources under `standards/` (`ct`, `sdtm`, `sdtmig`, `p21`, `xsl`)
...
- [ ] Select CLI crate (recommend: `clap` derive)
- [ ] Select CSV crate (`csv`)
- [ ] Select XML writing crate or strategy (recommend: `quick-xml` or custom writer)
- [ ] Select table UI crate (recommend: `comfy-table` or `tabled`)
- [ ] Select progress/spinner crate (recommend: `indicatif` or none)
- [ ] Select logging crates (`tracing`, `tracing-subscriber`)
- [ ] Prune config-related crates if unused (remove `figment`, `toml` if no config)
- [ ] Define log policy: levels, default level, and `-v` mapping
...
- [ ] Port domain processors in priority order (DM, AE, CM, DS, EX, LB, MH, PR, QS, SE, TA, TE, TS, VS, DA, IE, PE)
- [ ] Implement SUPPQUAL generation for non-LB domains
- [ ] Implement RELREC/RELSUB/RELSPEC synthesis
- [ ] Add regression tests for domain-specific transforms

Exit criteria:
- [ ] End-to-end domain processing works for mockdata
- [ ] Synthesized domains appear when required
