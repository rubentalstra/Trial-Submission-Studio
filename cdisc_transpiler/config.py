...
class TranspilerConfig:
    ...
    def __post_init__(self):
        ...
        if self.xpt_max_label_length < 1:
            raise ValueError(f"xpt_max_label_length must be positive, got {self.xpt_max_label_length}")
        if self.xpt_max_variables < 1:
            raise ValueError(f"xpt_max_variables must be positive, got {self.xpt_max_variables}")
        if self.qnam_max_length < 1:
            raise ValueError(f"qnam_max_length must be positive, got {self.qnam_max_length}")
