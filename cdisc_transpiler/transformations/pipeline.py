...
class TransformationPipeline:
    ...
    def execute(
        ...
        for i, transformer in enumerate(self.transformers):
            ...
            try:
                ...
                if result.applied:
                    ...
                    if result.warnings:
                        all_warnings.extend([f"{transformer_name}: {w}" for w in result.warnings])
                    if result.errors:
                        all_errors.extend([f"{transformer_name}: {e}" for e in result.errors])
                ...
                if self.fail_safe:
                    all_warnings.append(f"{transformer_name}: Caught exception, continuing (fail-safe mode)")
                else:
                    return TransformationResult(
                        data=current_data,
                        applied=True,
        ...
        if not applied_transformers:
            ...
            message = f"Applied {len(applied_transformers)} transformers: {', '.join(names)}"
    ...
    def __repr__(self) -> str:
        ...
        return f"TransformationPipeline({transformer_names}, fail_safe={self.fail_safe})"
