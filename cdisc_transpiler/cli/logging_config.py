"""Centralized logging configuration for CDISC Transpiler CLI.

This module provides structured logging with multiple verbosity levels,
rich formatting, and SDTM-specific context information.
...
class SDTMLogger:
    ...
    def log_final_stats(self) -> None:
        ...
        if self.verbosity >= LogLevel.VERBOSE:
            ...
            self.console.print(f"[dim]  Files processed: {self._stats['files_processed']}[/dim]")
            self.console.print(f"[dim]  Domains processed: {self._stats['domains_processed']}[/dim]")
            self.console.print(f"[dim]  Total records: {self._stats['records_processed']:,}[/dim]")
            if self._stats["warnings"] > 0:
                self.console.print(f"[dim yellow]  Warnings: {self._stats['warnings']}[/dim yellow]")
            if self._stats["errors"] > 0:
                self.console.print(f"[dim red]  Errors: {self._stats['errors']}[/dim red]")
...
_logger: SDTMLogger | None = None


def get_logger() -> SDTMLogger:
    """Get the global logger instance.
    
    Returns:
        The global SDTMLogger instance
    """
    global _logger
    if _logger is None:
        _logger = SDTMLogger()
...
def set_logger(logger: SDTMLogger) -> None:
...
def create_logger(console: Console | None = None, verbosity: int = 0) -> SDTMLogger:
    ...
    """
    logger = SDTMLogger(console, verbosity)
    set_logger(logger)
    return logger
