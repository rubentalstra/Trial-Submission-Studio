...
def write_variant_splits(
    ...
    """Write split XPTs for LB only.

    For LB, follow MSG sample: split by LBCAT into LBCH/LBHE/LBUR subsets.
    """
    ...
    # LB: category-based splits matching sample package
    if domain.code.upper() == "LB" and "LBCAT" in merged_dataframe.columns:
        cats = merged_dataframe["LBCAT"].astype("string").str.upper().str.strip()
        mapping = {
            "lbch": cats.eq("CHEMISTRY"),
            "lbhe": cats.eq("HEMATOLOGY"),
            # Remaining categories (URINALYSIS/OTHER) to LBUR
            "lbur": ~(cats.eq("CHEMISTRY") | cats.eq("HEMATOLOGY")),
        }
        for name, mask in mapping.items():
            subset = merged_dataframe.loc[mask].copy()
            if subset.empty:
                continue
            table = name.upper()
            split_path = split_dir / f"{name}.xpt"
            write_xpt_file(
                subset,
                domain.code,
                split_path,
...
def print_study_summary(
    ...
    """Print a summary of the study processing."""
    console.print()
    ...
    summary_table.add_column("Domain", style="cyan", justify="left")
    summary_table.add_column("Records", justify="right", style="green")

    if output_format in ("xpt", "both"):
        summary_table.add_column("XPT", justify="center")
    if output_format in ("xml", "both"):
        summary_table.add_column("Dataset-XML", justify="center")
    if generate_sas:
        summary_table.add_column("SAS", justify="center")
    summary_table.add_column("Extras", justify="left")
    ...
    for result in results:
        ...
        if output_format in ("xpt", "both"):
            row.append("[green]✓[/green]" if result.get("xpt_path") else "[dim]–[/dim]")
        if output_format in ("xml", "both"):
            row.append("[green]✓[/green]" if result.get("xml_path") else "[dim]–[/dim]")
        if generate_sas:
            row.append("[green]✓[/green]" if result.get("sas_path") else "[dim]–[/dim]")

        extras = ""
        split_paths = result.get("split_xpt_paths") or []
        if split_paths:
            extras = f"splits: {', '.join(Path(p).name for p in split_paths)}"
        row.append(extras)
    ...
    if errors:
        ...
        error_table.add_column("Domain", style="red")
        error_table.add_column("Error", style="yellow")
        for domain_name, error_msg in errors:
            error_table.add_row(
                domain_name,
...
def validate_command(
        ...
        for xpt_file in xpt_files:
            ...
            try:
                ...
                if issues:
                    ...
                    console.print(f"  [yellow]⚠[/yellow] {domain_code}: {error_count} errors, {warning_count} warnings")
                else:
    ...
    console.print()
    if error_count == 0:
        log_success(f"✓ Validation complete: {warning_count} warnings, {info_count} info messages")
    else:
        log_error(f"✗ Validation complete: {error_count} errors, {warning_count} warnings, {info_count} info")
