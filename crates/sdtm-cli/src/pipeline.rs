...
impl PipelineRunner {
    ...
    pub fn run(mut self) -> Result<StudyResult> {
        ...
        let ingest_span = info_span!(
            ...
            study_folder = %self.config.study_folder.display()
        );
        ...
        } = self.process_domains(&discovered, &process_context)?;
        ...
        // =========================================================================
        let mut report_map = self.validate(&frame_list, &self.config.study_id);
        ...
        let gating = gate_strict_outputs(output_formats, false, &conformance_reports);
        ...
        if self.config.output_formats.contains(&OutputFormat::Xpt) && allow_xpt {
...
struct ProcessContext<'a> {
    study_metadata: &'a StudyMetadata,
    suppqual_domain: &'a Domain,
    suppqual_exclusions: &'a BTreeSet<String>,
}
...
struct ProcessFileInput<'a> {
    ...
    pub context: &'a ProcessContext<'a>,
...
fn process_file(&self, input: &mut ProcessFileInput<'_>) -> Result<ProcessedFile> {
    let study_id = self.config.study_id.as_str();
        ...
        let AppliedStudyMetadata {
            ...
            code_to_base,
            derived_columns,
        } = if input.context.study_metadata.is_empty() {
            AppliedStudyMetadata::new(raw_table)
        } else {
            apply_study_metadata(raw_table, input.context.study_metadata)
        };
        ...
        let suppqual = info_span!("suppqual")
            .in_scope(|| -> Result<_> {
                ...
                let result = build_suppqual(&SuppqualInput {
                    ...
                    suppqual_domain: input.context.suppqual_domain,
                    source_df: &source.data,
                    mapped_df: Some(&mapped.data),
                    used_source_columns: &used,
                    study_id,
                    exclusion_columns: Some(input.context.suppqual_exclusions),
    ...
    let study_id = self.config.study_id.as_str();
        ...
        for domain_code in ordered_domains {
            ...
            for (path, variant) in files {
                ...
                match result {
                    Ok(processed) => {
                        *input_counts.entry(domain_key.clone()).or_insert(0) +=
                        ...
                        if multi_source {
                            ...
                            insert_frame(&mut processed_frames, processed.frame)
                        {
                            errors.push(format!("{}: {error}", path.display()));
                        }
                    }
