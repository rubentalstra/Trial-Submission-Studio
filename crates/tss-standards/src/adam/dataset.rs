...
impl CdiscVariable for AdamVariable {
    fn name(&self) -> &str {
        &self.name
    }
...
///
/// ```
/// use tss_standards::adam::{AdamDataset, AdamDatasetType};
///
...
impl CdiscDomain for AdamDataset {
    type Variable = AdamVariable;
    ...
    fn variables(&self) -> &[AdamVariable] {
        &self.variables
    }
}
    ...
    pub fn derived_variables(&self) -> Vec<&AdamVariable> {
        self.variables
            ...
            .filter(|v| {
                v.source
                    .as_ref()
                    .map(AdamVariableSource::is_derived)
                    .unwrap_or(false)
            })
            .collect()
    }
    ...
    pub fn sdtm_sourced_variables(&self) -> Vec<&AdamVariable> {
        self.variables
            ...
            .filter(|v| {
                v.source
                    .as_ref()
                    .map(AdamVariableSource::is_from_sdtm)
                    .unwrap_or(false)
            })
            .collect()
    }
