            ...
            body.rows(text_height + 8.0, filtered_vars.len(), |mut row| {
                ...
                row.col(|ui| {
                    let status_color = match status {
                        ...
                        VariableStatus::AutoGenerated => ui.visuals().hyperlink_color,
                    ...
                    if *status == VariableStatus::AutoGenerated {
...
///
/// By marking them as auto-generated upfront, we don't need to filter them
...
fn auto_accept_generated_variables(
    ...
    for var in &domain.variables {
        if is_auto_generated_variable(&var.name, var.role) {
            mapping_state.mark_auto_generated(&var.name);
            count += 1;
        }
    }
    count
...
fn sync_usubjid_from_subjid(ms: &mut MappingState) {
    ...
    if has_subjid {
        // USUBJID is derived from STUDYID + SUBJID, so mark it as auto-generated
        ms.mark_auto_generated("USUBJID");
    } else {
        // Clear USUBJID's auto-generated status when SUBJID is unmapped
...
fn show_source_mapping_inline(
    ...
    match status {
        ...
        VariableStatus::AutoGenerated => {
            // Auto-generated variables show info only, no actions needed
            ui.label(
                RichText::new(format!(
                    "{} Auto-generated",
                    egui_phosphor::regular::LIGHTNING
