...
use cdisc_map::{Mapping, MappingConfig};
...
pub fn generate_sas_program(
    ...
    let input_dataset = options
        .input_dataset
        .clone()
        .unwrap_or_else(|| format!("work.{}", domain.name.to_lowercase()));
    ...
    lines.push(format!("/* Domain: {} */", domain.name));
    ...
    if let Some(domain_col) = domain.column_name("DOMAIN") {
        lines.push(format!("    {domain_col} = \"{}\";", domain.name));
    }
    lines.push(String::new());
