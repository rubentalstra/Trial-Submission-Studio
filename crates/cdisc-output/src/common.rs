...
use cdisc_model::{CoreDesignation, Domain, Variable, VariableRole, VariableType};
...
pub fn is_reference_domain(domain: &Domain) -> bool {
    matches!(
        domain.class,
        Some(cdisc_model::DatasetClass::TrialDesign)
            | Some(cdisc_model::DatasetClass::StudyReference)
    )
}
