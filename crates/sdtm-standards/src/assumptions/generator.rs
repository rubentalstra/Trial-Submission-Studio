...
//!
//! This module ONLY generates rules for official P21 rule IDs. There are no
//! internal or custom rule IDs. All rule metadata comes from the loaded CSV files.
...
impl RuleGenerator {
    ...
    ///
    /// Only generates rules that have official P21 rule IDs from the loaded CSV.
    pub fn generate_rules_for_domain(
        ...
        for variable in &domain.variables {
            // Generate Core designation rules (SD0002, SD0056, SD0057)
            rules.extend(self.generate_core_rules(domain, variable));

            // Generate CT validation rules (CT2001, CT2002)
            rules.extend(self.generate_ct_rules(domain, variable, ct_registry));

            // Generate datetime format rules for *DTC variables (SD0003)
            rules.extend(self.generate_datetime_rules(domain, variable));

            // Generate sequence rules for *SEQ variables (SD0005)
...
pub struct RuleGenerationSummary {
    ...
    pub by_rule_id: std::collections::BTreeMap<String, usize>,
