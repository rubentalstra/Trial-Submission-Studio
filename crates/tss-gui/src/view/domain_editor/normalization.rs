...
use crate::theme::{
    ...
    MASTER_WIDTH, PRIMARY_500, SPACING_LG, SPACING_MD, SPACING_SM, SPACING_XS, SUCCESS, WARNING,
    WHITE,
};
...
fn view_rule_row<'a>(
    ...
    let dot_color = match var_status {
        VariableStatus::Accepted | VariableStatus::Suggested => SUCCESS,
        VariableStatus::AutoGenerated => PRIMARY_500,
        _ => GRAY_300,
    };
...
fn view_metadata_with_transform<'a>(
    ...
    column![
        ...
        transform_title,
        Space::new().height(SPACING_XS),
        text(&rule.description).size(12).color(GRAY_700),
        text(explanation).size(11).color(GRAY_500),
    ]
    .into()
}
...
fn get_transform_color(transform_type: &NormalizationType) -> Color {
    match transform_type {
        ...
        NormalizationType::UsubjidPrefix | NormalizationType::SequenceNumber => PRIMARY_500,
        NormalizationType::Iso8601DateTime
        | NormalizationType::Iso8601Date
        | NormalizationType::Iso8601Duration => Color::from_rgb(0.2, 0.6, 0.9),
        NormalizationType::StudyDay { .. } => Color::from_rgb(0.6, 0.4, 0.8),
        NormalizationType::CtNormalization { .. } => Color::from_rgb(0.2, 0.7, 0.5),
        NormalizationType::NumericConversion => Color::from_rgb(0.9, 0.5, 0.2),
