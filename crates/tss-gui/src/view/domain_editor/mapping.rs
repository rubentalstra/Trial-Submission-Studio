...
use crate::theme::{
    BORDER_RADIUS_SM, MASTER_WIDTH, SPACING_LG, SPACING_MD, SPACING_SM, SPACING_XS, button_primary,
    ...
    let domain = match state.domain(domain_code) {
        ...
        None => {
            return EmptyState::new(
                lucide::circle_alert().size(48).color(c.text_muted),
                "Domain not found",
            )
            .centered()
            .view();
...
fn view_variable_list_header<'a>(
    ...
    let stats = row![
        ...
        Space::new().width(4.0),
        text("mapped").size(11).color(c.text_muted),
    ]
...
fn view_variable_item<'a>(
    ...
    let status_icon: Element<'a, Message> = match status {
        VariableStatus::Accepted => lucide::circle_check()
            .size(12)
            .color(c.status_success)
            .into(),
        VariableStatus::AutoGenerated => lucide::settings().size(12).color(c.text_muted).into(),
        VariableStatus::Suggested => lucide::lightbulb().size(12).color(c.status_warning).into(),
        VariableStatus::NotCollected => lucide::ban().size(12).color(c.text_disabled).into(),
        VariableStatus::Omitted => lucide::eye_off().size(12).color(c.text_disabled).into(),
        VariableStatus::Unmapped => lucide::circle().size(12).color(c.text_disabled).into(),
    };

    // Core badge
    let (badge_text, badge_color) = match var.core {
        Some(CoreDesignation::Required) => (Some("Req"), Some(c.status_error)),
        Some(CoreDesignation::Expected) => (Some("Exp"), Some(c.status_warning)),
        Some(CoreDesignation::Permissible) => (Some("Perm"), Some(c.text_muted)),
        None => (None, None),
    };
...
fn view_mapping_status<'a>(
    ...
    let title = text("Mapping Status").size(14).color(c.text_secondary);
    ...
    let status_content: Element<'a, Message> = match status {
        VariableStatus::Accepted => {
            if let Some((col, conf)) = domain.mapping.accepted(&var.name) {
                ...
                StatusCard::new(lucide::circle_check().size(16).color(c.status_success))
                    .title("Mapped to:")
                    .value(col)
                    .description(format!("{}% confidence", conf_pct))
                    .background(c.status_success_light)
                    .border_color(c.status_success)
                    .view()
            } else {
        ...
        VariableStatus::AutoGenerated => {
            StatusCard::new(lucide::settings().size(16).color(c.text_muted))
        ...
        VariableStatus::Suggested => {
            if let Some((col, conf)) = domain.mapping.suggestion(&var.name) {
                ...
                StatusCard::new(lucide::lightbulb().size(16).color(c.status_warning))
                    ...
                    .background(c.status_warning_light)
                    .border_color(c.status_warning)
                    .view()
            } else {
                view_status_unmapped()
            }
        ...
        VariableStatus::NotCollected => {
            ...
            StatusCard::new(lucide::ban().size(16).color(c.text_muted))
                .value("Not Collected")
                .description(reason)
                .view()
        }
        VariableStatus::Omitted => StatusCard::new(lucide::eye_off().size(16).color(c.text_muted))
            .value("Omitted")
...
fn view_status_unmapped<'a, M: Clone + 'a>() -> Element<'a, M> {
    let c = colors();
    StatusCard::new(lucide::circle().size(16).color(c.text_disabled))
        .value("Not Mapped")
...
fn view_controlled_terminology<'a>(
    ...
    let title_row = row![
        lucide::list().size(14).color(c.accent_primary),
    ...
    let content: Element<'a, Message> = if let Some(resolved) = resolved {
        ...
        let codelist_name = text(&codelist.name).size(13).color(c.text_primary);
        let codelist_code_text = text(format!("({})", ct_code)).size(11).color(c.text_muted);

        let border_default = c.border_default;
        let error = c.status_error;
        let error_light = c.status_error_light;
        ...
        let extensible_badge = if codelist.extensible {
            container(text("Extensible").size(10).color(text_muted))
                .padding([2.0, 6.0])
            ...
            container(text("Non-extensible").size(10).color(error))
                .padding([2.0, 6.0])
                .style(move |_: &Theme| container::Style {
                    background: Some(error_light.into()),
                    border: Border {
                        radius: 4.0.into(),
                        color: error,
                        width: 1.0,
        ...
        let terms_header = row![
            ...
            text("Meaning").size(10).color(text_muted),
        ]
        .padding(SPACING_SM);

        terms_list = terms_list.push(terms_header);
        terms_list = terms_list.push(rule::horizontal(1).style(move |_| rule::Style {
        ...
        for term in terms.iter().take(show_count) {
            ...
            let term_row = row![
                text(&term.submission_value)
                    ...
                    .color(accent)
                    .width(Length::Fixed(350.0)),
                text(meaning).size(12).color(text_secondary),
            ]
            .padding(SPACING_SM)
            .align_y(Alignment::Center);
        ...
        column![
            header_row,
            Space::new().height(SPACING_SM),
            text("Allowed Values:").size(11).color(text_muted),
            Space::new().height(SPACING_XS),
            terms_container,
        ]
        .into()
        ...
        row![
            lucide::triangle_alert().size(12).color(c.status_warning),
            Space::new().width(SPACING_SM),
...
fn view_source_column_picker<'a>(
    ...
    if column_options.is_empty() {
        return container(
            row![
                lucide::info().size(14).color(text_muted),
                Space::new().width(SPACING_SM),
    ...
    .style(move |_: &Theme, status| {
        let border_color = match status {
            pick_list::Status::Active => accent,
            pick_list::Status::Hovered => text_disabled,
    ...
    column![
        text("Map to Source Column").size(14).color(text_secondary),
        Space::new().height(SPACING_SM),
        picker,
        Space::new().height(SPACING_XS),
        text(helper_text).size(11).color(text_muted),
    ]
    .into()
}
...
fn view_not_collected_inline_edit<'a>(
    ...
    let accent = c.accent_primary;
    let border_default = c.border_default;
    let border_error = c.border_error;
    let bg_elevated = c.background_elevated;
    ...
    let error_msg: Element<'a, Message> = if is_empty {
        ...
        text("Reason too long").size(11).color(error_color).into()
    } else {
        Space::new().height(0.0).into()
    };
    ...
    column![
        ...
        Space::new().height(SPACING_SM),
        row![
            text("Reason *").size(12).color(c.text_muted),
            Space::new().width(Length::Fill),
