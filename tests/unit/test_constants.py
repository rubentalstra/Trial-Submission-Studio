...
class TestDefaults:
    ...
    def test_output_format_values(self):
        ...
        assert Defaults.OUTPUT_FORMAT in valid_formats, f"OUTPUT_FORMAT must be one of {valid_formats}"
...
class TestConstraints:
    ...
    def test_define_xml_version(self):
        ...
        assert pattern.match(Constraints.DEFINE_XML_VERSION), "Version must be X.Y.Z format"
    
    def test_dataset_xml_version(self):
        """Test Dataset-XML version string format."""
        pattern = re.compile(r"^\d+\.\d+\.\d+$")
        assert pattern.match(Constraints.DATASET_XML_VERSION), "Version must be X.Y.Z format"
...
class TestPatterns:
    ...
    def test_sdtm_variable_name_pattern(self):
        ...
        for name in invalid_names:
            assert not pattern.match(name), f"{name} should be invalid SDTM variable name"
