...
class TestDomainFileDiscovery:
    ...
    def test_can_find_lb_variant_files(self, study_folder):
        ...
        lb_files = [
            f for f in lb_files if "Items" not in f.name and "CodeLists" not in f.name
        ]
    ...
    def test_can_identify_domain_variants(self, study_folder):
        ...
        domain_files = [
            f
            for f in csv_files
            if "Items" not in f.name
            and "CodeLists" not in f.name
            and "CSV2SAS" not in f.name
        ]
        ...
        for f in domain_files:
            ...
            if len(parts) > 0:
                ...
                if domain_part.startswith("LB"):
                    ...
                    domains.add(
                        domain_part[:2] if len(domain_part) >= 2 else domain_part
                    )
