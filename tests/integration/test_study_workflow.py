...
class TestStudyWorkflowWithGDISC:
    ...
    def test_study_processing_creates_output_directories(
        self, study_folder, output_dir, container
    ):
    ...
    def test_can_list_csv_files_in_study(self, study_folder):
        ...
        domain_files = [
            f for f in csv_files if any(d in f.name.upper() for d in ["DM", "AE", "LB"])
        ]
