...
class TestStudyCommand:
    ...
    def test_study_with_default_options(self, runner, study_folder, tmp_path):
        ...
        # Check exit code
        assert result.exit_code == 0, f"Command failed with output: {result.output[:500]}"
    ...
    def test_study_with_xpt_format(self, runner, study_folder, tmp_path):
        ...
        assert not xml_dir.exists(), "XML directory should not be created with --format xpt"
    ...
    def test_study_with_xml_format(self, runner, study_folder, tmp_path):
        ...
        xpt_dir = output_dir / "xpt"
        assert not xpt_dir.exists(), "XPT directory should not be created with --format xml"
...
class TestStudyCommandWithGDISC:
    ...
    def test_study_full_processing(self, runner, study_folder, tmp_path):
        ...
        assert len(xpt_files) >= 10, f"Expected at least 10 XPT files, got {len(xpt_files)}"
        
        # Check for key domains
        expected_domains = ["dm.xpt", "ae.xpt", "lb.xpt", "vs.xpt"]
        for domain_file in expected_domains:
